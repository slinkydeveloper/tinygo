	...
	for instIndex := 0; instIndex < len(bb.instructions); instIndex++ {
		...
		case llvm.Call:
			...
			case callFn.name == "runtime.alloc":
				...
				// Most of the complexity below is caused by not knowing exactly
				// what the type of this global should be, as runtime.alloc does
				// not have type information.

				// Get the requested memory size to be allocated.
				size := operands[1].Uint()

				// Try to find the LLVM type by looking for a bitcast. There
				// should be only one bitcast at most.
				var llvmType llvm.Type
				for use := inst.llvmInst.FirstUse(); !use.IsNil(); use = use.NextUse() {
					user := use.User()
					if !user.IsABitCastInst().IsNil() {
						if !llvmType.IsNil() {
							return nil, mem, r.errorAt(inst, errors.New("more than one bitcast use of runtime.alloc"))
						}
						llvmType = user.Type()
					}
				}
				i8ptrType := inst.llvmInst.Type()
				if llvmType.IsNil() {
					// Fall back to a i8, which is probably incorrect. That's
					// why we extend it to an array below.
					llvmType = i8ptrType
				}

				// The bitcast could be casting to an array of a certain type,
				// for example for use in a slice. If so, the requested object
				// size will be a multiple of the element type and we can find
				// the global type by creating the backing array type.
				elementType := llvmType.ElementType()
				elementSize := r.targetData.TypeAllocSize(elementType)
				if elementSize != size {
					if size/elementSize*elementSize != size {
						panic("size not a multiple of elementSize")
					}
					initializerType := llvm.ArrayType(elementType, int(size/elementSize))
					llvmType = llvm.PointerType(initializerType, 0)
				}

				// Now finally create the object.
				alloc := object{
					llvmType:   llvmType,
